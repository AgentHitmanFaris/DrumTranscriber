...
class DrumTranscriber:
    def __init__(self):
        try:
            self.model = tf.keras.models.load_model(SETTINGS["SAVED_MODEL_PATH"], compile=False, safe_mode=False)
        except TypeError:
            # Fallback for older keras versions that don't verify safe_mode
